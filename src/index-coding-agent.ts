#!/usr/bin/env node
/**
 * Entry point for the Azure DevOps MCP Server (Coding Agent Edition)
 *
 * This variant is optimized for LLM coding agents that have local repository access.
 * It provides only work item management and commit/branch linking tools.
 */

import { createCodingAgentServer } from './server-coding-agent';
...
async function main() {
  try {
    // Create the coding agent server with configuration
    const server = createCodingAgentServer(getConfig());
    ...
    process.stderr.write(
      'Azure DevOps MCP Server (Coding Agent Edition) running on stdio\n',
    );
  } catch (error) {
...
export * from './server-coding-agent';
